...
from util import prediction, read, stats, data_process

if __name__ == "__main__":
    x_data, y_data = data_process(r'.\NASA\D\PC5.arff')
    print(len(x_data), len(y_data))
    x_train, x_test, y_train, y_test = train_test_split(x_data, y_data, test_size=0.2, random_state=4)
    tree_list = ensemble_tree(50)
    ftree_list = building_under_sampling_data(x_train, y_train, 0.06, tree_list)
